...
def test_mqa_init_rejects_non_single_kv_head() -> None:
    config = LlamaConfig(
        dim=32,
        n_layers=2,
        n_heads=4,
...
def test_mqa_prefill_and_decode_smoke() -> None:
    config = _tiny_mqa_config()
...
def test_llama_backbone_with_mqa_factory_smoke() -> None:
    config = _tiny_mqa_config()
    model = LlamaBackbone(config)
    model.set_attention(MQAFactory(config))
