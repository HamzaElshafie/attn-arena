...
def _repeat_single_kv_head(
    k: torch.Tensor,
    v: torch.Tensor,
    n_heads: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Expand single KV head to query-head count for compute fallback."""
    ...
    if n_kv_heads != 1:
        raise ValueError("MQA fallback expects a single KV head.")

    k_expanded = (
        k.unsqueeze(3)
        .expand(batch_size, seq_len, n_kv_heads, n_heads, head_dim)
...
def _sdpa_with_mqa_kv(
    ...
    *,
    ...
    """Run SDPA for MQA with grouped-KV fast path and safe fallback.
    ...
    try:
        ...
        k_expanded, v_expanded = _repeat_single_kv_head(
            k.transpose(1, 2),
            v.transpose(1, 2),
            n_heads=q.shape[1],
        )
...
class MultiQueryAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig) -> None:
        ...
        if self.n_kv_heads != 1:
            raise ValueError("n_kv_heads must be == 1 in MQA.")
    ...
    def prefill(
        ...
        attn_output = _sdpa_with_mqa_kv(q, k, v, attention_mask=attention_mask)
    ...
    def decode(
        ...
        attn_output = _sdpa_with_mqa_kv(q, k, v, attention_mask=None)
        attn_output = attn_output.transpose(1, 2).contiguous().view(batch_size, seq_len, -1)
        return AttentionOutput(self.wo(attn_output))
...
class MQAFactory:
    """Create a fresh MQA module per layer to avoid parameter sharing."""
    ...
    def create(self, layer_idx: int) -> AttentionModule:
        _ = layer_idx
        return MultiQueryAttention(self.config)
